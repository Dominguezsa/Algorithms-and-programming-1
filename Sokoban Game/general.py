...
def juego_mostrar(juego, indice, ancho, alto, pistas):
    ...
    horizontal = (
                         ancho - columnas_grilla * CUADRADO) // 2  # "horizontal" es el espacio en el eje x que hay entre el borde de la ventana y la grilla
    vertical = (alto - len(
        juego) * CUADRADO) // 2  # "vertical" es el espacio en el eje y que hay entre el borde de la ventana y la grilla
    gamelib.draw_text('SokoBan', 50, 20)
    gamelib.draw_text('P: pista', medio, (alto + alto - vertical // 2) // 2)
